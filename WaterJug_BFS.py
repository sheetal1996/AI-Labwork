...
def dfs( start, goal, depth=20): #NOTE --> The value of depth is set to 10 for depth limit search, remove it for normal DFS.
	...
	while True:
		...
		if node.state==goal:
			flag=1
		elif goal[0]=='*' and node.state[1]==goal[1]:
			flag=1
		elif goal[1]=='*' and node.state[0]==goal[0]:
			flag=1
		if flag==1:
			moves = []
			sol=[]
