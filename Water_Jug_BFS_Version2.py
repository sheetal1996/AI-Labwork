...
def dfs( start, goal, depth=10): #NOTE --> The value of depth is set to 10 for depth limit search, remove it for normal DFS.
	...
	while True:
		...
		if node.state==goal:
			flag=1
		elif goal[0]==999 and node.state[1]==goal[1]:
			flag=1
		elif goal[1]==999 and node.state[0]==goal[0]:
			flag=1
		if flag==1:
			moves = []
			sol=[]
...
starting_state=[0,0]
n1=input('Enter the capacity of first jug: ')
n2=input('Enter the capacity of second jug: ')
print "-----Enter Goal value------"
o1=input('First jug (Enter 999 for any value):  ')
o2=input('Second jug (Enter 999 for any value) :  ')
goal_state=[o1,o2]
