...
def expand_node( node):
	...
	expanded_nodes = []
	expanded_nodes.append( create_node( move_up( node.state ), node, "u", node.depth + 1, h(move_up( node.state ))+node.depth +1) )
	expanded_nodes.append( create_node( move_down( node.state ), node, "d", node.depth + 1, h(move_down( node.state ))+node.depth+1 ) )
	expanded_nodes.append( create_node( move_left( node.state ), node, "l", node.depth + 1, h(move_left( node.state ))+node.depth +1) )
	expanded_nodes.append( create_node( move_right( node.state), node, "r", node.depth + 1, h(move_right( node.state ))+node.depth +1) )
	# Filter the list and remove the nodes that are impossible (move function returned None)
	expanded_nodes = [node for node in expanded_nodes if node.state != None] #list comprehension!
	return expanded_nodes

def a_star( start, goal = goal_state ):
...
def sort(nodes):
	for j in range(len(nodes)):
			for i in range(len(nodes)-1):
				#NOTE --> Here, g(n) is the depth of node i.e. distance covered so far and h(n) is the number of misplaced tiles that gives an estimation value to reach solution
...
def main():
	...
	#Below line is for random problem generator
	#r.shuffle(starting_state)
	print starting_state
	moves, result = a_star( starting_state, goal_state )
	print "Initial state: "
