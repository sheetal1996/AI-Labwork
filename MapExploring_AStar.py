...
def a_star( start, goal, depth=10):
	...
	while True:
		...
		print '( ',node.value, ', ',node.cost+h[node.value],'); ',
...
def sort(nodes):
	for j in range(len(nodes)):
			for i in range(len(nodes)-1):
				if (nodes[i].cost+h[nodes[i].value]) > (nodes[i+1].cost+h[nodes[i+1].value]):
					temp=nodes[i]
					nodes[i]=nodes[i+1]
					nodes[i+1]=temp
...
def main():
	...
	if starting_state not in RomaniaMap.keys() or goal_state not in RomaniaMap.keys():
	...
	if starting_state==goal_state:
		...
		cost,result = a_star( starting_state, goal_state )
		if result == None:
